...
int check_message_queue() {
    ...
    for(int i=0;i<3;i++){
        ...
        if (msgctl(msgQueueID, IPC_STAT, &buf) == -1) {
            perror("Error retrieving message queue information");
            return -1;
        }
        if(buf.msg_qnum == 0){
            Cooking_recipe_writer(msgQueueID, recipes[i*2].recipeContent);
            Cooking_recipe_writer(msgQueueID, recipes[i*2+1].recipeContent);
        }
    }
    return 0;
}
